...
public interface HeadOfficeMapper {
	...
	int HeadOfficeElectronicPaymentSave(Map<String, Object> paramMap); 								// 본사 -> 전자결재 관리 -> 구매요청 품의서 메인 저장(전자결재 maintable)
	...
	List<Map<String, Object>> EvaluationFormTypes(); 												// 인사 -> 평가 -> 평가폼
	List<Map<String, Object>> EvaluationTypeList();  												// 인사 -> 평가 -> 타입조회
	int EvaluationTypeInsert(Map<String, Object> paramMap);											// 인사 -> 평가 -> 타입저장
	int EvaluationTypeUpdate(Map<String, Object> paramMap);											// 인사 -> 평가 -> 타입수정
